...
class Generate(commands.Cog):
    ...
    async def generate(self, interaction: discord.Interaction, category: str):
        ...
        # ── Channel embed (minimal) ────────────────────────────────────────────
        channel_embed = discord.Embed(
            color=color,
            description=f"<@{interaction.user.id}> generated an account!",
        )
        channel_embed.set_author(name="Account Generated", icon_url=guild_icon)
        channel_embed.set_footer(text=f"Generator • {left} left in stock")
        if banner_url:
            channel_embed.set_image(url=banner_url)

        # ── DM embed (full) ────────────────────────────────────────────────────
        dm_description = "\n".join(detail_lines) if detail_lines else "\u200b"
        title = f"Generated Account — {username}" if username else f"Generated Account — {CATEGORY_LABELS[category]}"

        dm_embed = discord.Embed(color=color, title=title, description=dm_description)
        dm_embed.set_author(name=interaction.guild.name, icon_url=guild_icon)
